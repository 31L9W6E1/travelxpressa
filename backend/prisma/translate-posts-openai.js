  ...
  for (const arg of argv) {
    if (arg.startsWith('--locales=')) {
      ...
      const categories = value
        ...
        .map((item) => item.trim().toLowerCase())
        .filter(Boolean)
        .filter((item) => item === 'news' || item === 'blog');
...
async function main() {
  ...
  if (posts.length === 0) {
    console.log('No published posts found for translation.');
    return;
  }
  ...
  console.log(`Publish status: ${options.publish ? 'published' : 'draft'}`);
  console.log(`Created translations: ${created}`);
