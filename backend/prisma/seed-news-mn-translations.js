...
/**
 * Seed/Upsert Mongolian (mn) translations for published NEWS posts.
 *
 * This does NOT call any external API. It uses curated manual translations.
 *
 * Usage (example):
 *   DATABASE_URL='postgresql://...' node prisma/seed-news-mn-translations.js
 */

const { PrismaClient } = require('@prisma/client');
const translations = require('./manual-translations/news_mn');
...
async function main() {
  ...
  if (!Array.isArray(translations) || translations.length === 0) {
    throw new Error('No translations found in prisma/manual-translations/news_mn.js');
  }
  ...
  for (const item of translations) {
    ...
    if (post.category !== 'news' || post.status !== 'published') {
      console.warn(`[skip] Not a published news post: ${slug}`);
      continue;
    }
